...
export default {
  ...
  //   message: {
  //   get () {
  //     return  this.$store.state.specialInspection.editPramas
  //   },
  //   set (value) {
  ...
  methods: {
    ...
    async getSpecialList() {
      const { data: res } = await http.get("/api/findSpecialList");
      console.log(res);
    ...
    checkedChange(item) {
      ...
      this.$store.commit("setData", this.checkedArr);
      console.log("store", this.$store.state.specialInspection.editPramas);
      console.log(JSON.parse(JSON.stringify(this.checkedArr)));
      //  return JSON.parse(JSON.stringify(this.checkedArr))
    },
    goBack() {},
    goDetail() {
      if (this.checkedArr.length !== 0) {
        this.$router.push({ name: "InspectionDetail" ,query:{isEdit:true}});
      }else{
        Toast.fail('至少选择一项');
      }
